...
fun interface PhonePlanner {
    suspend fun nextDecision(context: TaskContext, uiSnapshot: UiSnapshot?): PlannerValidation
}

object PhoneGoalVerifier {
    fun verify(goal: TaskGoal, uiSnapshot: UiSnapshot?, context: TaskContext): GoalVerification {
        if (uiSnapshot == null) return GoalVerification(GoalVerificationStatus.NOT_COMPLETED, "Screen context পাওয়া যায়নি।")
        val packageName = uiSnapshot.packageName.orEmpty().lowercase()
        ...
        val criteriaSatisfied = goal.successCriteria.isEmpty() || goal.successCriteria.all { criterion ->
            ...
            uiSnapshot.nodes.any { element ->
                element.text.orEmpty().lowercase().contains(normalized) ||
                    element.contentDescription.orEmpty().lowercase().contains(normalized)
            }
        }
...
class PhoneAgent(
    ...
    private suspend fun executeLoop(
        ...
        repeat(maxPlannerCalls.coerceAtLeast(1)) { callIndex ->
            ...
            val verification = PhoneGoalVerifier.verify(goal, uiSnapshot, taskContext)
            ...
            val decisionValidation = planner.nextDecision(
                taskContext.copy(
                    currentScreenFingerprint = uiSnapshot?.fingerprint(),
                    currentApp = uiSnapshot?.packageName
                ),
                uiSnapshot
            )
            ...
            val actionKey = "${uiSnapshot?.fingerprint()}|${decision.action}|${decision.arguments}"
    ...
    private suspend fun executeDecision(decision: PlannerDecision): ActionResult {
        ...
        if (name in setOf("open_app", "list_apps", "youtube_search", "web_search", "open_settings", "set_brightness", "call_phone", "search_contacts", "set_alarm", "set_timer")) {
