...
object PhonePlannerContract {
    ...
    private val requiredArguments = mapOf(
        ...
        "call_phone" to setOf("phone_number"),
        "search_contacts" to setOf("query"),
        "set_alarm" to setOf("hour", "minute"),
        "set_timer" to setOf("seconds")
        ,"find_text" to setOf("text"),
        "find_view" to setOf("view_id"),
        "tap" to setOf("x", "y"),
        "double_tap" to setOf("x", "y"),
        "long_press" to setOf("x", "y"),
        "swipe" to setOf("x1", "y1", "x2", "y2"),
        "drag" to setOf("x1", "y1", "x2", "y2"),
        "set_text" to setOf("target_text", "text"),
        "clear_text" to setOf("target_text")
    )
    ...
    fun validateFunctionCall(
        ...
        if (name == "set_alarm") {
            val hour = arguments["hour"].asIntOrNull() ?: return invalid("Hour অবৈধ।")
            val minute = arguments["minute"].asIntOrNull() ?: return invalid("Minute অবৈধ।")
            if (hour !in 0..23) return invalid("Hour 0 থেকে 23-এর মধ্যে হতে হবে।")
            if (minute !in 0..59) return invalid("Minute 0 থেকে 59-এর মধ্যে হতে হবে।")
        }
        if (name == "set_timer") {
            val seconds = arguments["seconds"].asIntOrNull() ?: return invalid("Seconds অবৈধ।")
            if (seconds <= 0) return invalid("Seconds 0 থেকে বড় হতে হবে।")
        }
    ...
    private fun Any?.asIntOrNull(): Int? = when (this) {
        is Number -> toInt()
        else -> this?.toString()?.toDoubleOrNull()?.toInt() ?: this?.toString()?.toIntOrNull()
    }
